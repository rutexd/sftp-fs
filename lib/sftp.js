...
const sftp = (fs, connection, stream) => {
    ...
    connection.addAction("open", async (requestId, pathname, flags, attrs) => {
        pathname = await fs.normalize(connection.client.session, pathname);
    ...
    connection.addAction("rename", async (requestId, oldPathname, newPathname) => {
        oldPathname = await fs.normalize(connection.client.session, oldPathname);
        newPathname = await fs.normalize(connection.client.session, newPathname);
    ...
    connection.addAction("symlink", async (requestId, linkPathname, targetPathname) => {
        linkPathname = await fs.normalize(connection.client.session, linkPathname);
        targetPathname = await fs.normalize(connection.client.session, targetPathname);
